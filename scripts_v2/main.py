    ...
    def wait_till_end(self, num_cmds: int):
        ...
        with tqdm.tqdm(total=total_lines) as pbar:
            while not("Done" in status):
                ...
                if "No Sockets found" in screen_status:
...
"""
        ...
        ssh_cmd = f"screen -dm bash -c 'sh {self.workdir}/jobs/{job_file} > arbiter_log.txt 2>&1'"
        res = run_remote_public_ip([
            ssh_cmd + " && echo 'Screen session started'" # This is needed otherwise the screen -dm doesn't work :shrug:
...
"""
            for vm, bin_list in self.binary_mapping.items():
                ...
                # Copy the logs back
                    _script += f"""
$SCP_CMD {self.dev_ssh_user}@{vm.public_ip}:{self.remote_workdir}/logs/{repeat_num}/{bin}.log {self.remote_workdir}/logs/{repeat_num}/{bin}.log
$SCP_CMD {self.dev_ssh_user}@{vm.public_ip}:{self.remote_workdir}/logs/{repeat_num}/{bin}.err {self.remote_workdir}/logs/{repeat_num}/{bin}.err
"""
