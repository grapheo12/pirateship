...
def main(deploy_config, registry_name, resource_group, image_name, cluster_key, base_port, docker_ssh, local): 
   ...
   for platform in extractConfigArgs:
      ...
      print("Launching "+ launch_count+ " node containers")
      for i in range(0, launch_count): 
        nodepool_container_tag = nodepool_container_tag + str(i)
        cu.launchDeployment("docker/arm-template-standard.json", resource_group, nodepool_container_tag, registry_name, image_name, "docker/cluster_key.pub", token , location, base_port, docker_ssh, local) 
        ip = cu.obtainIpAddress(resource_group, nodepool_container_tag, local) 
        public_ips[nodepool_container_tag] = "127.0.0.1" 
        ...
        docker_ssh = docker_ssh + 1

      # Launch Client Containers
      location = platform['location']
      launch_count = int(platform["clientpool_count"])
      print("Launching "+ launch_count + " client containers")
      for i in range(0, launch_count): 
        client_container_tag = client_container_tag + str(i)
        cu.launchDeployment("docker/arm-template-standard.json", resource_group, client_container_tag, registry_name, image_name, "docker/cluster_key.pub", token , location, base_port, docker_ssh, local) 
   ...
   dev_vm_ip = public_ips["clientpool_vm0"] 
   dev_user = "pftadmin"
